...
x0_test = r_test * np.cos(theta_test)
y0_test = r_test * np.sin(theta_test)
z0_test = z_test + 0*r_test
...
assert z0_test[0]<radius
r_loc = np.sqrt(radius**2 - z0_test[0]**2)
rho_ref = len(x)/(4/3*pi*radius**3)
lam_loc = rho_ref*pi*r_loc**2
e_ref = rho_ref/(epsilon_0) * (
        r_test/2*(np.abs(r_test)<r_loc)
      + r_loc**2/r_test/2*(np.abs(r_test)>=r_loc))
plt.figure(102)
plt.plot(r_test, ex_test*x0_test/r_test
        + ey_test*y0_test/r_test)
