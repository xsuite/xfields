...
trackers = []
p_tests = []
for disable_crab in [True, False]:
    ...
    line_b1.vars['on_crab1'] = 0 # Crabs in b1 off, irrelevant because test particles do not go through them
    line_b4.vars['on_crab1'] = 0
    ...
    xf.configure_orbit_dependent_parameters_for_bb(tracker_b1,
                            particle_on_co=twiss_b1.particle_on_co)

    p_test = tracker_b1.build_particles(x_norm=1, zeta=np.linspace(-0.3, 0.3, 100),
                                        nemitt_x=nemitt_x, nemitt_y=nemitt_y,
                                        at_element='mbas2.1l1')

    tracker_b1.track(p_test, ele_stop='mbas2.1r1')

    dfs.append(bb_dfs)
    p_tests.append(p_test)
    trackers.append(tracker_b1)
