...
n_slots = h_list[0]/bunch_spacing_buckets
bucket_length = circumference / h_list[0]
filling_scheme = np.zeros(int(np.amin(h_list) / bunch_spacing_buckets))
...
n_bunches = int(np.sum(filling_scheme))

bunch_numbers = np.linspace(0, n_bunches - 1, n_bunches, dtype=int)
print(bunch_numbers)
...
if nprocs >= 2:
    ...
    for rank in range(nprocs):
        pipeline_manager.add_particles(f'bunches_{rank}', rank)
...
particles.init_pipeline(f'bunches_{0}')


#print(f'rank {my_rank}: Initialising multitracker')
...
for i_turn in range(n_turns):
    plt.plot(particles.zeta, particles.x, 'x')
    plt.show()


    #if i_turn > 0: pr.enable()
    line.track(particles, num_turns=1)
    #if i_turn > 0: pr.disable()

    mean_x_xt[i_turn] = np.mean(particles.x[:n_macroparticles])
    mean_y_xt[i_turn] = np.mean(particles.y[:n_macroparticles])

    if i_turn % 50 == 0:
        print(f'Turn: {i_turn}')
        print(mean_x_xt[i_turn])
