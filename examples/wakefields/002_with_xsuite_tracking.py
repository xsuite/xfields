...
xt_map = xt.LineSegmentMap(
    length=circumference, betx=beta_x, bety=beta_y,
    qx=accQ_x, qy=accQ_y, momentum_compaction_factor=alpha,
...
for i_turn in range(n_turns):
    ...
    if plot_on:
        ...
        ax01.plot(
            wf.slicer.zeta_centers.T,
            xf_slicer_after.mean('px').T - wf.slicer.mean('px').T, 'x', color=color_list[i_turn])
