...
def test_bunch_monitor_hdf5(test_context):
    ...
    monitor = xf.CollectiveMonitor(
        base_file_name=f'test_monitor_{my_rank}',
        backend='hdf5',
    ...
    particles = xt.Particles(
        ...
        x=np.concatenate([x_coord * (bid + 1) for bid in bunch_selection]),
        px=np.concatenate([px_coord * (bid + 1) for bid in bunch_selection]),
        y=np.concatenate([y_coord * (bid + 1) for bid in bunch_selection]),
        py=np.concatenate([py_coord * (bid + 1) for bid in bunch_selection]),
        zeta=np.concatenate([zeta_coord * (bid + 1) - bunch_spacing_zeta * bid
                             for bid in bunch_selection]),
        delta=np.concatenate(
            [delta_coord * (bid + 1) for bid in bunch_selection]),
    )

    monitor._reconfigure_for_parallel(n_procs, my_rank)
...
def test_slice_monitor_hdf5(test_context):
    ...
    for i_bunch, bid in enumerate(bunch_selection):
        zeta_coord = zeta_coord_base * (bid + 1) - bid * bunch_spacing_zeta

        if len(bunch_selection) > 1:
            bin_min = monitor.slicer.zeta_centers[i_bunch, n_slice] - dzeta / 2
            bin_max = monitor.slicer.zeta_centers[i_bunch, n_slice] + dzeta / 2
        else:
    ...
    with h5py.File(monitor._slices_filename, 'r') as h5file:
        for i_bunch, bid in enumerate(bunch_selection):
            ...
            assert np.allclose(slice_data['mean_x'][:, n_slice],
                               np.mean(x_coord_bunch[i_bunch]))
            assert np.allclose(slice_data['mean_px'][:, n_slice],
                               np.mean(px_coord_bunch[i_bunch]))
            assert np.allclose(slice_data['mean_y'][:, n_slice],
                               np.mean(y_coord_bunch[i_bunch]))
            assert np.allclose(slice_data['mean_py'][:, n_slice],
                               np.mean(py_coord_bunch[i_bunch]))
            assert np.allclose(slice_data['mean_zeta'][:, n_slice],
                               np.mean(zeta_coord_bunch[i_bunch]))
            assert np.allclose(slice_data['mean_delta'][:, n_slice],
                               np.mean(delta_coord_bunch[i_bunch]))

            assert np.allclose(slice_data['sigma_x'][:, n_slice],
                               np.std(x_coord_bunch[i_bunch]))
            assert np.allclose(slice_data['sigma_px'][:, n_slice],
                               np.std(px_coord_bunch[i_bunch]))
            assert np.allclose(slice_data['sigma_y'][:, n_slice],
                               np.std(y_coord_bunch[i_bunch]))
            assert np.allclose(slice_data['sigma_py'][:, n_slice],
                               np.std(py_coord_bunch[i_bunch]))
            assert np.allclose(slice_data['sigma_zeta'][:, n_slice],
                               np.std(zeta_coord_bunch[i_bunch]))
            assert np.allclose(slice_data['sigma_delta'][:, n_slice],
                               np.std(delta_coord_bunch[i_bunch]))

            epsn_x = (np.sqrt(np.var(x_coord_bunch[i_bunch]) *
                              np.var(px_coord_bunch[i_bunch]) -
                              np.cov(x_coord_bunch[i_bunch],
                                     px_coord_bunch[i_bunch])[
                                  0, 1])
                      * beta * gamma)
            epsn_y = (np.sqrt(np.var(y_coord_bunch[i_bunch]) *
                              np.var(py_coord_bunch[i_bunch]) -
                              np.cov(y_coord_bunch[i_bunch],
                                     py_coord_bunch[i_bunch])[
                                  0, 1])
                      * beta * gamma)
            epsn_zeta = (np.sqrt(np.var(zeta_coord_bunch[i_bunch]) *
                                 np.var(delta_coord_bunch[i_bunch]) -
                                 np.cov(zeta_coord_bunch[i_bunch],
                                        delta_coord_bunch[i_bunch])[
                                     0, 1]) * beta * gamma)
            ...
            assert np.allclose(slice_data['num_particles'][:, n_slice],
                               n_macroparticles_slice_bunch[i_bunch])
...
def test_particle_monitor_hdf5(test_context):
    ...
    particles = xt.Particles(
        ...
        x=np.random.random(n_macroparticles) * (my_rank + 1),
        px=np.random.random(n_macroparticles) * (my_rank + 1),
        y=np.random.random(n_macroparticles) * (my_rank + 1),
        py=np.random.random(n_macroparticles) * (my_rank + 1),
        zeta=np.random.random(n_macroparticles) * (my_rank + 1),
        delta=np.random.random(n_macroparticles) * (my_rank + 1),
    )
