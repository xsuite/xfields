#ifndef XFIELDS_BEAMBEAM3DBASE_H
#define XFIELDS_BEAMBEAM3DBASE_H
...
/*
    printf("\tSigma_xx_ip=%.10e\n", Sig_11_0);
    printf("\tSigma_yy_ip=%.10e\n", Sig_33_0);
    printf("\tSigma_pxpx_ip=%.10e\n", Sig_22_0);
    printf("\tSigma_pypy_ip=%.10e\n", Sig_44_0);
    printf("\tSigma_xx_cp=%.10e\n", Sig_11);
    printf("\tSigma_yy_cp=%.10e\n", Sig_33);
*/
...
void BeamBeamBiGaussian3DBase_track_local_particle(BeamBeamBiGaussian3DBaseData el, 
   ...
   // tt = clock();
    ...
    const Sigmas Sigmas_0_star = BeamBeamBiGaussian3DBaseData_getp_Sigmas_0_star(el);
    const double min_sigma_diff = BeamBeamBiGaussian3DBaseData_get_min_sigma_diff(el);
    const double threshold_singular = 
	    BeamBeamBiGaussian3DBaseData_get_threshold_singular(el);
    const int N_slices = BeamBeamBiGaussian3DBaseData_get_num_slices(el);
    const double delta_x = BeamBeamBiGaussian3DBaseData_get_delta_x(el);
    const double delta_y = BeamBeamBiGaussian3DBaseData_get_delta_y(el);
    const double x_CO  = BeamBeamBiGaussian3DBaseData_get_x_CO(el);     
    const double px_CO = BeamBeamBiGaussian3DBaseData_get_px_CO(el);
    const double y_CO = BeamBeamBiGaussian3DBaseData_get_y_CO(el);
    const double py_CO = BeamBeamBiGaussian3DBaseData_get_py_CO(el);
    const double sigma_CO = BeamBeamBiGaussian3DBaseData_get_sigma_CO(el);
    const double delta_CO = BeamBeamBiGaussian3DBaseData_get_delta_CO(el);
    const double Dx_sub = BeamBeamBiGaussian3DBaseData_get_Dx_sub(el); 
    const double Dpx_sub = BeamBeamBiGaussian3DBaseData_get_Dpx_sub(el);
    const double Dy_sub =BeamBeamBiGaussian3DBaseData_get_Dy_sub(el);
    const double Dpy_sub =BeamBeamBiGaussian3DBaseData_get_Dpy_sub(el);
    const double Dsigma_sub =BeamBeamBiGaussian3DBaseData_get_Dsigma_sub(el);
    const double Ddelta_sub =BeamBeamBiGaussian3DBaseData_get_Ddelta_sub(el);
    /*gpuglmem*/ const double* N_part_per_slice_arr = 
	    BeamBeamBiGaussian3DBaseData_getp1_N_part_per_slice(el, 0);
    /*gpuglmem*/ const double* x_slices_star_arr = 
	    BeamBeamBiGaussian3DBaseData_getp1_x_slices_star(el, 0);
    /*gpuglmem*/ const double* y_slices_star_arr = 
	    BeamBeamBiGaussian3DBaseData_getp1_y_slices_star(el, 0);
...
/*
            printf("[beambeam3d] [%d] sigmas:\n", part->ipart);
      	    printf("\tx_slice_star=%.10e\n", x_slice_star); 
     	    printf("\ty_slice_star=%.10e\n", y_slice_star);  
     	    printf("\tsigma_star=%.10e\n", sigma_star);    
    	    printf("\tsigma_slice_star=%.10e\n", sigma_slice_star);    
   	    printf("\tSz_i=%.10e\n", S);    
*/
    	    // Propagate sigma matrix
