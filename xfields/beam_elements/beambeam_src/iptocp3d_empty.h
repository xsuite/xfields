...
//   tt = clock() - tt;
   ...
   /*gpuglmem*/ const double* x_bb_centroid  = IPToCP3D_emptyData_getp_x_bb_centroid(el);
   /*gpuglmem*/ const double* y_bb_centroid  = IPToCP3D_emptyData_getp_y_bb_centroid(el);
   /*gpuglmem*/ const double* z_bb_centroid  = IPToCP3D_emptyData_getp_z_bb_centroid(el);
   /*gpuglmem*/ const double* px_bb_centroid = IPToCP3D_emptyData_getp_px_bb_centroid(el);
   /*gpuglmem*/ const double* py_bb_centroid = IPToCP3D_emptyData_getp_py_bb_centroid(el);

   // boosted centroid of other full beam at IP
   /*gpuglmem*/ const double* x_full_bb_centroid  = IPToCP3D_emptyData_getp_x_full_bb_centroid(el);
   /*gpuglmem*/ const double* y_full_bb_centroid  = IPToCP3D_emptyData_getp_y_full_bb_centroid(el);

   // boosted centroid of this beam slice at IP
   /*gpuglmem*/ const double* z_centroid  = IPToCP3D_emptyData_getp_z_centroid(el);

   // get additional params of the beambeam element
   const int64_t slice_id                 = IPToCP3D_emptyData_get_slice_id(el);
   const int64_t is_sliced                = IPToCP3D_emptyData_get_is_sliced(el);
   const unsigned int use_strongstrong    = IPToCP3D_emptyData_get_use_strongstrong(el);
