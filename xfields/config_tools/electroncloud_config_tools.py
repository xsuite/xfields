...
def get_electroncloud_fieldmap_from_h5(filename, tau_max=None, buffer=None, ecloud_name="e-cloud"):
    ...
    if tau_max is None:
        ...
        zg = ff["grid/zg"][()]
        nz = len(zg)
        min_index = np.argmin(np.abs(zg + np.abs(tau_max))) - 1 # add one index to make sure range is included
        max_index = np.argmin(np.abs(zg - np.abs(tau_max))) + 1 #

        if min_index < 0 or max_index > nz:
            raise Exception(f"Range ({-np.abs(tau_max):.4f}, {np.abs(tau_max):.4f}) not in maximum range of z_grid: ({zg[0]:.4f},{zg[-1:]:.3f}) of file: {filename}")
    ...
    memory_estimate = (ix2 - ix1) * (iy2 - iy1) * (iz2 - iz1) * 8 * 8 * 1.e-9# (in GB), 8 bytes per double-precision number
    ...
    scale = [1., fieldmap.dx, fieldmap.dy, fieldmap.dz, 
             fieldmap.dx * fieldmap.dy, fieldmap.dx * fieldmap.dz, fieldmap.dy * fieldmap.dz, 
    ...
    for iz in range(iz1, iz2):
        ...
        phi_slice = ff[f"slices/slice{iz}/phi"][ix1:ix2, iy1:iy2,:].transpose(1,0,2)
        for ll in range(8):
            phi_slice[:,:,ll] *= scale[ll]
        index_offset = 8 * nx * ny * (iz - iz1)
        len_slice = phi_slice.shape[0] * phi_slice.shape[1] * phi_slice.shape[2]
        fieldmap._phi_taylor[index_offset:index_offset+len_slice] = phi_slice.flatten()
    ###################################################################################
    # for iz in range(iz1, iz2):
    #     if (iz-iz1)/nz > kk:
    #         while (iz-iz1)/nz > kk:
...
def insert_electronclouds(eclouds, fieldmap=None, line=None):
    ...
    for name in eclouds.keys():
        s = eclouds[name]["s"]
        length = 0.
        line.insert_element(element=xf.ElectronCloud(length=length, fieldmap=fieldmap, _buffer=fieldmap._buffer), name=name, at_s=s)

def config_electronclouds(line, twiss=None, ecloud_info=None, shift_to_closed_orbit=False, subtract_dipolar_kicks=False, fieldmaps=None, ecloud_strength=1.):
    ...
    if subtract_dipolar_kicks:
        ...
        for key in fieldmaps.keys():
            ...
            dipolar_kicks[key] = electroncloud_dipolar_kicks_of_fieldmap(fieldmap=fieldmap, p0c=line.particle_ref.p0c)

    length_factor = ecloud_strength / ( line.particle_ref.p0c * line.particle_ref.beta0 )
    part = twiss["particle_on_co"].copy()
    for ii, el_name in enumerate(twiss["name"]):
        if 'ecloud' in el_name:
            ecloud_type = el_name.split(".")[1]  ## naming format is "ecloud.ecloud_type.sector.index_in_sector", e.g.  ecloud.mb.78.38
            length = ecloud_info[ecloud_type][el_name]["length"]*length_factor
            ...
            if shift_to_closed_orbit:
                ...
                line.elements[ii].tau_shift = part.zeta[0] / (part.beta0[0] * part.rvv[0])
...
def electroncloud_dipolar_kicks_of_fieldmap(fieldmap=None, p0c=None):
    ...
    ecloud = xf.ElectronCloud(length=1, fieldmap=fieldmap, _buffer=fieldmap._buffer)
    ecloud.track(part)
    px = part.px[0]
    py = part.py[0]
    ptau = part.ptau[0]
...
def full_electroncloud_setup(line=None, ecloud_info=None, filenames=None, context=None, tau_max=None, subtract_dipolar_kicks=True, shift_to_closed_orbit=True):

    buffer = context.new_buffer()
    fieldmaps = { ecloud_type : get_electroncloud_fieldmap_from_h5(filename=filename, buffer=buffer, tau_max=tau_max, ecloud_name=ecloud_type) for (ecloud_type, filename) in filenames.items()}

    for ecloud_type, fieldmap in fieldmaps.items():
        print(f"Inserting \"{ecloud_type}\" electron clouds...")
        insert_electronclouds(ecloud_info[ecloud_type], fieldmap=fieldmap, line=line)

    tracker = xt.Tracker(_context=context, line=line, _buffer=buffer)
    twiss_without_ecloud = tracker.twiss()
    config_electronclouds(line, twiss=twiss_without_ecloud, ecloud_info=ecloud_info, subtract_dipolar_kicks=subtract_dipolar_kicks, shift_to_closed_orbit=shift_to_closed_orbit, fieldmaps=fieldmaps, ecloud_strength=1)
