...
#ifndef XO_CONTEXT_CUDA /* Not on CUDA */
...
void compute_slice_moments(ParticlesData particles, int64_t* particles_slice, double* moments, int n_slices, int threshold_n_macroparticles) {
    ...
    _Pragma("omp parallel default(none) firstprivate(n_part,n_slices,n_first_moments) shared(particles,moments,particles_slice)")
    ...
    #ifdef XO_CONTEXT_CPU_OPENMP
    _Pragma("omp parallel default(none) firstprivate(n_part,n_slices,n_first_moments,n_second_moments) shared(particles,moments,particles_slice)")
    #endif // XO_CONTEXT_CPU_OPENMP
    {
